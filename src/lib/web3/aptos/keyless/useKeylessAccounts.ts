...
import { EphemeralKeyPairEncoding, isValidEphemeralKeyPair, validateEphemeralKeyPair } from "./ephemeral";
import { aptosClient, LocalStorageKeys } from "../constants";
import { EncryptedScopedIdToken } from "./types";
import { validateIdToken } from "./idToken";
import { createUser, getUserByEmail, upsertUser } from "@/server/users";
        ...
        (set, get, store) => ({
            ...
            ...({
                ...
                switchKeylessAccount: async (idToken: string) => {
                    ...
                    try {
                        activeAccount = await aptosClient.deriveKeylessAccount({
                            ephemeralKeyPair,
                            jwt: idToken,
                            proofFetchCallback,
                        });
                        ...
                        get().disconnectKeylessAccount();
                        store.persist.rehydrate();
                        if (activeAccount && decodedToken.email) {
                            ...
                            if (!user) {
                                ...
                                console.log("🚀 ~ switchKeylessAccount: ~ activeAccount:", activeAccount.accountAddress.toString())
                            } else {
