...
module nft_addr::test_nft {
    ...
    #[test(creator = @0x1)]
    #[expected_failure(abort_code = 393219, location = nft_addr::nft)] // Correct abort code
    public entry fun test_get_non_existent_nft(creator: &signer) {
        account::create_account_for_test(signer::address_of(creator));

        // Initialize an NFT to ensure the MetadataMap is created
        let metadata = string::utf8(b"https://example.com/nft_for_test");
        let token_id = nft::test_initialize_nft(creator, metadata);

        // Ensure the MetadataMap exists
        assert!(nft::exists_metadata_map(signer::address_of(creator)), 1);
